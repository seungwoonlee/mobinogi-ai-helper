...
- 개발 로드맵과 문서 지도: `00-로드맵.md`
- 연결 감지와 상태 설계: `02-연결-상태-설계.md`
- 명령 실행과 안전 설계: `03-명령-실행과-안전-설계.md`
- UI 상세 설계: `04-UI-상세설계.md`
...
그다음 `@@` 뒤에 대사를 입력하면 **보낼 내용을 먼저 보여주고 확인**을 받은 뒤 게임 채팅으로
전송합니다. `@@문구`와 `@@ 문구`는 똑같이 동작해요.

```text
> @@고마워요
보낼 말: 고마워요 😍 (행동: /하트)
다른 플레이어에게 보이는 게임 채팅이에요.
보낼까요? [y/N] y
```

`y` 한 글자만 확인으로 인정하고 그 밖의 입력은 취소예요. 이 도구는 스크립트나 파이프로는
채팅을 보내지 않고 터미널에서 직접 실행할 때만 동작합니다.

> **중요: 처음에는 모든 전송이 "확인 불가"로 안내돼요.** 게임이 채팅 성공을 어떤 응답으로
> 알려주는지 아직 확인하지 못해서(`config/known_responses.json`이 비어 있어요), 이 도구는 성공을
> 추측하지 않아요. 실제로는 전달됐을 수 있으니 **게임 채팅창에서 직접 확인**하세요. 이 상태에서는
> 자동 행동(`/하트` 등)도 실행되지 않고, 다음 전송 때 "이미 전송됐을 수 있어요. 채팅창에서
> 확인했나요?"를 한 번 더 물어요. 응답 형식을 확인하는 방법은 `tools/capture_sample.py`
> (직접 실행, 실제 채팅 1건이 나가요)와 `05-구현계획-2단계-CLI-기반.md`를 보세요.

모든 대사에는 이모지 하나가 붙습니다. 감사·인사·축하·사과·응원·웃김·슬픔처럼
뜻이 분명하면 어울리는 행동도 이어서 실행해요(성공 응답을 확인한 뒤에만). 예를 들어
`고마워요`에는 😍와 `/하트`, `죄송해요`에는 😓와 `/사과1`이 붙습니다. `안 고마워`처럼 부정이
붙거나 `사과 팔아요`처럼 다른 뜻이면 행동은 붙지 않아요. 일반적인 문장은 😊만 붙고 행동은
실행하지 않아요.
